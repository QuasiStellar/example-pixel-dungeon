...
public class HeroSelectScene extends PixelScene {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				if (cls != null) {
					...
					ExamplePixelDungeon.scene().addToFront(w);
				}
			}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				ExamplePixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
	...
	private class GameOptions extends Component {
		...
		protected void createChildren() {
			...
			StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
				...
				protected void onClick() {
					if (!Badges.isUnlocked(Badges.Badge.VICTORY) && !DeviceCompat.isDebug()){
						ExamplePixelDungeon.scene().addToFront( new WndTitledMessage(
					...
					ExamplePixelDungeon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
							...
							if (positive && seed != -1){
								...
								for (GamesInProgress.Info info : GamesInProgress.checkAll()){
									if (info.customSeed.isEmpty() && info.seed == seed){
										SPDSettings.customSeed("");
										icon.resetColor();
										ExamplePixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
										return;
									}
								}
			...
			StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
				...
				protected void onClick() {
					...
					if (diff > 24*HOUR){
						ExamplePixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
						return;
					}

					for (GamesInProgress.Info game : GamesInProgress.checkAll()){
						if (game.daily){
							ExamplePixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
							return;
						}
					}
					...
					ExamplePixelDungeon.scene().addToFront(new WndOptions(
							icon,
							Messages.get(HeroSelectScene.class, "daily"),
							diff > 0 ?
			...
			StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
				...
				protected void onClick() {
					...
					ExamplePixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
